...
<head>
    ...
    <style>
        * { font-family: 'Plus Jakarta Sans', 'Tajawal', system-ui, sans-serif; }
        ...
        .otp-input:focus { outline: none; border-color: {{ $primaryColor }}; box-shadow: 0 0 0 4px rgba({{ $primaryRgb }}, .1); background: #fff; }
        .btn-otp {
            ...
            box-shadow: 0 12px 28px -8px {{ $primaryColor }}; transition: all .25s ease; font-size: .95rem;
        }
        .btn-otp:hover { filter: brightness(1.08); transform: translateY(-2px); }
        .shield-icon { width: 64px; height: 64px; border-radius: 18px; background: rgba({{ $primaryRgb }}, .08); display: inline-grid; place-items: center; font-size: 1.7rem; margin-bottom: 16px; }
    </style>
</head>
<body>
    ...
    <div class="card-2fa">
        <div class="text-center mb-4">
            <div class="shield-icon" style="color:{{ $primaryColor }}">
                <i class="bi bi-shield-lock-fill"></i>
            </div>
            <h1 class="h4 fw-bolder mb-1" style="letter-spacing:-.02em">@lang('two_factor')</h1>
            <p class="small mb-0" style="color:#64748b;line-height:1.5">@lang('two_factor_desc')</p>
        </div>

        @if (session('status'))
            <div class="alert alert-success py-2 small" style="border-radius:12px;background:rgba(16,185,129,.08);color:#059669;border:none;font-weight:600"><i class="bi bi-check-circle-fill me-1"></i>{{ session('status') }}</div>
        @endif

        @if (session('error'))
            <div class="alert alert-warning py-2 small" style="border-radius:12px;background:rgba(245,158,11,.1);color:#d97706;border:none;font-weight:600"><i class="bi bi-exclamation-triangle-fill me-1"></i>{{ session('error') }}</div>
        @endif
        ...
        <div class="text-center mt-3">
            <form method="POST" action="{{ route('admin.2fa.resend') }}" class="d-inline">
                @csrf
                <button type="submit" class="btn btn-link btn-sm text-decoration-none fw-bold" style="color:{{ $primaryColor }}">
        ...
        <div class="text-center mt-2">
            <a href="{{ route('admin.login') }}" class="text-decoration-none small fw-bold" style="color:#64748b">@lang('back_to_login')</a>
        </div>
    </div>
</body>
</html>
